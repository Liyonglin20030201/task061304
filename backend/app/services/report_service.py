...
async def generate_report(
    ...
    store_filter_sales = ""
    store_filter_inv = ""
    params: dict = {"start": request.start_date, "end": request.end_date}
    if request.store_ids is not None:
        store_filter_sales = "AND s.store_id = ANY(:store_ids)"
        store_filter_inv = "AND i.store_id = ANY(:store_ids)"
    ...
    if request.report_type == "sales_summary":
        ...
        WHERE s.sale_date BETWEEN :start AND :end {store_filter_sales}
        ...
        WHERE i.snapshot_date BETWEEN :start AND :end {store_filter_inv}
